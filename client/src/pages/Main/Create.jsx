...
const Create = ({...props}) => {
    ...
    const handleSubmit = (event) => {
        ...
        AxiosHelper.createProduct(data).then((res) => {
            if (res.status == 200) {
                props.setModalShow(false);
                props.setToast({ content: 'Product created', variant: 'success', title: 'Success', show: true });
            }
        }).catch((err) => {
            if (!err?.response) {
    ...
    return (
        <ModalU show={props.modalShow} size={''} title={'Product [New]'}
            actionbtntext={'Save'}
